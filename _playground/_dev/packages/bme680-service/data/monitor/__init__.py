...
"""
BME680 Temperature, Pressure, Humidity & Gas Sensor - Official Bosch Implementation

This is the official Bosch BME680 sensor library implementation.
Original source code from Bosch for the BME680 environmental sensor.
...
- Temperature change-over-time tracking for gas sensor stability

The library uses smbus2 for I2C communication on Raspberry Pi hardware.

**Gas Heater Temperature Control:**
The BME680 uses an internal heater for VOC measurements. The heater temperature and
duration are controlled via set_gas_heater_temperature() and set_gas_heater_duration().
Temperature tracking during burn-in shows when the sensor reaches thermal equilibrium.

Usage:
    from monitor2 import BME680, I2C_ADDR_PRIMARY
    
    sensor = BME680(I2C_ADDR_PRIMARY)
    sensor.set_gas_heater_temperature(320)  # Heater to 320°C
    sensor.set_gas_heater_duration(150)     # Duration 150ms
    sensor.select_gas_heater_profile(0)
    
    if sensor.get_sensor_data() and sensor.data.heat_stable:
        print(f"Temp: {sensor.data.temperature}°C")
        print(f"Humidity: {sensor.data.humidity}%")
        print(f"Pressure: {sensor.data.pressure} hPa")
...
#<
    ...
    def set_baselines(self, burn_in_time=300, verbose=False): #> 
        ...
        try:
            while curr_time - start_time < burn_in_time:
                ...
                if self.get_sensor_data() and self.data.heat_stable:
                    gas_data = self.data.gas_resistance
                    hum_data = self.data.humidity
                    ...
                    gas_burn_data.append( self.data.gas_resistance )
                    hum_burn_data.append( self.data.humidity )
                    
                    if len(gas_burn_data) > 1:
                        
                        first = gas_burn_data[1]
                        prior = gas_burn_data[-2]
                        curr = gas_burn_data[-1]
                        
                        change_from_prior = ( (gas_burn_data[-1] - gas_burn_data[-2]) / gas_burn_data[-2]) * 100
                        change_from_orrig = ( (gas_burn_data[-1] - first) / first) * 100 
                        
                        
                        print( '{:.2f} - {:00.3f} (P:{:00.2f})(O:{:00.2f})'.format( curr_time - start_time,gas_data,change_from_prior , change_from_orrig            ))
                      
                        # print( '{:.2f} - (Curr: {}) (Prior: {}) (Variance: {:.2f})'.format( curr_time - start_time, curr, prior, ( (gas_burn_data[-1] - gas_burn_data[-2]) / gas_burn_data[-2]) * 100) )
                    else:
                        print( '{:.2f} - '.format(curr_time - start_time) ) 
                    
                    
                    
                    
                    # if verbose == True:
                    #     print('{0:.2f} - BLGas: {1:.2f} Ohms, BLHum: {2:.2f}'.format(curr_time - start_time,gas_data, hum_data))
            ...
            burn_in_values = len( gas_burn_data )
            if burn_in_values < 50:
                averaged_qty = burn_in_values
            else:
                averaged_qty = 50
            
            gas_baseline = sum(gas_burn_data[-averaged_qty:]) / averaged_qty
            hum_baseline = sum(hum_burn_data[-averaged_qty:]) / averaged_qty
            
            self.baselines = dict(gas = gas_baseline , hum = hum_baseline)
            self.baseline_status = 1
    ...
    def get_gas_baseline(self): #> 
        if self.baseline_status == 1:
            return self.baselines.get("gas")
        elif self.baseline_status == -1:
            return False #<
            
    def get_hum_baseline(self): #> 
        if self.baseline_status == 1:
            return self.baselines.get("hum")
        elif self.baseline_status == -1:
            return False 
    #<
   
    def get_aqs_avg(self): #> 
        return self.aqs_avg
    #<
    
    def calculate_aqs_avg(self, records_to_average=10, verbose=False): #> 
        start_time = time.time()
        curr_time = time.time()
        air_quality_data = []
        aqs_count = 1
        
        gas_baseline = self.baselines.get("gas")
        if not gas_baseline:
	        raise ValueError('gas_baseline not calculated')
        
        hum_baseline = self.baselines.get("hum")
        if not hum_baseline:
            raise ValueError('hum_baseline not calculated')
            
        # Sets the balance between humidity and gas in air_quality_score (25:75, humidity:gas)
        hum_weighting = 0.25 
        
        try:
            while aqs_count < records_to_average:
                if self.get_sensor_data() and self.data.heat_stable:
                    curr_time = time.time()
                    ...
                    aqs_count = len( air_quality_data )
                    
                    if aqs_count >= records_to_average:
                        self.aqs_avg = sum(air_quality_data[-records_to_average:]) / records_to_average
                        ...
                        if verbose == True:
                            print('Insufficent data: Calculated {} of {}'.format( aqs_count, records_to_average ))
    ...
    def update_saved_aqs_avg(self, file_path, keyword, new_line): #>
        # print(file_path, keyword, new_line)
    ...
    def check_saved_variable_value(self, target_variable): #>
        """parse the saved_variables file for given variable.

        target_variable is the name of the variable.

        """
        with open('/home/pi/printer_data/config/saved_variables.cfg', 'r') as file:
            lines = file.readlines()
        for i, line in enumerate(lines):
...
def percent_variance(original_value, new_value):
    ...
    #             line = 'aqs_avg = {0:.2f}'.format(self.aqs_avg)
    ...
    #                     target_file.write('aqs_avg = {0:.2f}'.format(self.aqs_avg))
    ...
    # def replace2(source_file_path, pattern, substring):

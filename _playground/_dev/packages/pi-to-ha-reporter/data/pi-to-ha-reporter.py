...
def on_connect(client, userdata, flags, reason_code, properties):
    # VERSION2 callback API: (client, userdata, flags, reason_code, properties)
    ...
    if reason_code == 0:
        ...
        print_line('! Connection error with reason code {}'.format(str(reason_code)), error=True)
        print_line('MQTT Connection error with reason code {}'.format(str(reason_code)), error=True, sd_notify=True)
...
def on_disconnect(client, userdata, reason_code, properties):
    # VERSION2 callback API: (client, userdata, reason_code, properties)
    global mqtt_client_connected
    mqtt_client_connected = False
...
def on_publish(client, userdata, mid, reason_code, properties):
    # VERSION2 callback API: (client, userdata, mid, reason_code, properties)
...
def on_subscribe(client, userdata, mid, reason_code, granted_qos, properties):
    # VERSION2 callback API: (client, userdata, mid, reason_code, granted_qos, properties)
...
# Default to user's local share directory (works for any user, not just 'pi')
default_update_flag_filespec = os.path.join(
    os.path.expanduser('~'), '.local', 'share', 'pi-to-ha-reporter', 'lastupd.date')
...
# report our RPi values - supports seconds (preferred) or minutes (legacy)
# New: interval_in_seconds (10-1800 seconds, default: 10)
# Legacy: interval_in_minutes (1-30 minutes, converts to seconds)
min_interval_in_seconds = 10
max_interval_in_seconds = 1800  # 30 minutes
default_interval_in_seconds = 10

# Check for new seconds-based config first
if config['Daemon'].get('interval_in_seconds'):
    interval_in_seconds = config['Daemon'].getint('interval_in_seconds', default_interval_in_seconds)
else:
    # Fall back to legacy minutes-based config
    interval_in_minutes = config['Daemon'].getint('interval_in_minutes', 5)
...
if (interval_in_seconds < min_interval_in_seconds) or (interval_in_seconds > max_interval_in_seconds):
    print_line('ERROR: Invalid "interval_in_seconds" found in configuration file: "config.ini"! Must be [{}-{}] seconds. Fix and try again... Aborting'.format(
...
if interval_in_seconds < 300:  # less than 5 minutes
    K_LD_CPU_USE_JSON = "cpu.load_1min_prcnt"
elif interval_in_seconds < 900:  # less than 15 minutes
...
def startPeriodTimer():
    ...
    endPeriodTimer = threading.Timer(
        float(interval_in_seconds), periodTimeoutHandler)

...
@test "date prompt: message displayed once (no duplicates)" {
    load_iMenu
    ...
    # Capture stderr to check for duplicate messages
    local stderr_output
    stderr_output=$(imenu_date "test_name" "Enter date:" "2024-01-01 12:00:00" 2>&1 >/dev/null)
    
    # Verify the complete initial display structure
    assert_date_display_structure "$stderr_output" "newline" "Enter date:" "2024-01-01 12:00:00"
}

@test "date prompt: message displayed once in inline mode (no duplicates)" {
    load_iMenu
    ...
    # Verify the complete initial display structure
    assert_date_display_structure "$stderr_output" "inline" "Enter date:" "2024-01-01 12:00:00"
}

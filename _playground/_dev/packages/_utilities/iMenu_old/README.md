...
## iPrompt - Single Prompt Abstraction

The `iprompt_run` function provides a unified interface for running individual prompts. It supports two usage patterns:

### Array Reference Method

Define a step as an array and pass the array name:
...
### One-Liner Method (Direct Arguments)

Pass arguments directly without creating an array:

```bash
result=$(iprompt_run "password_result" "password" "ℹ️  Enter your password:")
```

### One-Liner Examples by Prompt Type
...
```bash
result=$(iprompt_run "text_result" "text" "ℹ️  Enter your name:" "--initial" "John Doe")
```

**Number with min/max:**
```bash
result=$(iprompt_run "number_result" "number" "ℹ️  Enter your age:" "--min" "1" "--max" "120")
```

**Select with options:**
```bash
result=$(iprompt_run "select_result" "select" "ℹ️  Pick a color:" "Red" "Green" "Blue")
```

**Multiselect with preselection:**
```bash
result=$(iprompt_run "colors_result" "multiselect" "ℹ️  Pick colors:" "Red" "Green" "Blue" "--preselect" "0 2")
```

**Toggle with default:**
```bash
result=$(iprompt_run "toggle_result" "toggle" "ℹ️  Enable notifications?" "true")
```

**Confirm:**
```bash
result=$(iprompt_run "confirm_result" "confirm" "ℹ️  Do you agree?" "false")
```

**List with initial value:**
```bash
result=$(iprompt_run "list_result" "list" "ℹ️  Enter tags:" "--initial" "tag1, tag2, tag3")
```

**Autocomplete:**
```bash
result=$(iprompt_run "autocomplete_result" "autocomplete" "ℹ️  Search country:" "USA" "Canada" "Mexico" "--limit" "10")
```

**Date:**
```bash
result=$(iprompt_run "date_result" "date" "ℹ️  Pick a date:" "--initial" "2024-01-01" "--mask" "%Y-%m-%d")
```
...
- `--mask` - Date format mask (for date)

### Format

```bash
iprompt_run "name" "type" "message" [options...]
```

- **name**: Response key name (stored in `_IMENU_RESPONSES_MAP`)
- **type**: Prompt type (`text`, `password`, `select`, etc.)
- **message**: Display message
- **options**: Additional options (choices, flags, etc.)

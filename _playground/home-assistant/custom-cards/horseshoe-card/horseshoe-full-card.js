/*
*
* Card      : horseshoe-card.js (renamed from flex-horseshoe-card for identification)
...
import {
  ...
  */  
  ...
  set hass(hass) { // This is a safe and fast method  // Set ref to hass, use "_"for the name ;-)
    ...
    // Ensure min, max, and state are valid numbers
    const min = Number(this.config.horseshoe_scale?.min) || 0;
    const max = Number(this.config.horseshoe_scale?.max) || 100;
    const numState = Number(state) || 0;
    
    // Calculate value with NaN protection
    let val = this._calculateValueBetween(min, max, numState);
    if (isNaN(val) || !isFinite(val)) {
      val = 0;
    }
    val = Math.min(Math.max(val, 0), 1); // Clamp between 0 and 1
    ...
    this.dashArray = `${score} ${total}`;
      ...
      if (strokeStyle == 'fixed') {
        ...
        // Ensure val is valid before calculating offset
        const safeVal = (isNaN(val) || !isFinite(val)) ? 0 : Math.max(0, Math.min(1, val));
        this.color1_offset = `${Math.round((1-safeVal)*100)}%`;
  ...
  // If text property is defined, use it instead of entity state
  // Check for both 'text' and 'text:' properties (YAML can use either)
  const staticText = item.text !== undefined ? item.text : (item['text:'] !== undefined ? item['text:'] : null);
  if (staticText !== null && staticText !== undefined && staticText !== '') {
    // Debug: Log when rendering static text
    if (staticText === 'B:' || staticText === 'S:') {
      console.log('Rendering static text label:', staticText, 'at', x, y, 'item:', item);
    }
    return svg`
    <text @click=${e => this.handlePopup(e, this.entities[item.entity_index || 0])}>
  ...
  * event for further processing.
  ...
  _calculateValueBetween(start, end, val) {
    // Handle NaN, null, undefined, or invalid inputs
    const numStart = Number(start);
    const numEnd = Number(end);
    const numVal = Number(val);
    
    // Return 0 if any input is invalid (NaN, null, undefined)
    if (isNaN(numStart) || isNaN(numEnd) || isNaN(numVal)) {
      return 0;
    }
    ...
    return (Math.min(Math.max(numVal, numStart), numEnd) - numStart) / (numEnd - numStart);
  }
  ...
  customElements.define('horseshoe-card', FlexHorseshoeCard);
